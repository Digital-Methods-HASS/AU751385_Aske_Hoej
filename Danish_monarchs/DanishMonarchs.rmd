...
read_csv and read_csv2 are tidyverse functions, because they make the tipple
...
kings <- read_csv("data/danish_monarchs.csv")
...
```{r}
# YOUR CODE
duration <- as.numeric(kings_duration$durationreign)
class(duration)
...
```{r}
# YOUR CODE
avg_duration <- mean(kings_duration$durationreign, na.rm = TRUE)
...
longestreign <- kings %>%
  arrange(desc(kings_duration$durationreign)) %>% 
  slice(1:3)

kings_duration <- kings %>% 
  mutate(days=kings_duration$durationreign*365)
